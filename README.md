...
- 探针支持聊天、OpenAI 图片文生图、图生图、Responses 图片和 Gemini/Banna 图片接口
...
- `/probe`：保存 Provider 配置，按探针类型执行聊天首 Token、图片生成/编辑、Gemini/Banna 图片接口探测；模型列表统一来自当前配置的 `/v1/models` 或手动选中结果。
